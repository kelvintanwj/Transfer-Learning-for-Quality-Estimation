...
class MonoTransQuestModel:
    def __init__(
        ...
        elif isinstance(args, MonoTransQuestArgs):
            self.args = args
    ...
    def eval_model_mcdropout(
        ...
        result, model_outputs, wrong_preds = self.evaluate_mcdropout(
            eval_df, output_dir, multi_label=multi_label, verbose=verbose, silent=silent, wandb_log=wandb_log, **kwargs
        )
        self.results.update(result)
    ...
    def evaluate_mcdropout(
        ...
        forward_passes = 10
        dropout_predictions = np.empty((0, len(eval_dataset), self.num_labels))
        ...
        for i in range(forward_passes):
            ...
            # changing Dropout layers to train
            ...
            dropout_predictions = np.vstack((dropout_predictions, preds[np.newaxis, :, :]))
        
        # calculating preds as mean of stacked predictions
        preds = np.mean(dropout_predictions, axis=0)
    ...
    def predict(self, to_predict, multi_label=False):
        ...
        if not multi_label and self.args.onnx:
            ...
            if self.config.output_hidden_states:
                ...
                """
                # TODO:
                1. Add enable_dropout function
                2. Rather than running predict once per data batch, we run multiple forward passes
                whilst leaving model.train on dropout layers
                3. calculate mean of all forward passes predictions as preds
                https://stackoverflow.com/questions/63285197/measuring-uncertainty-using-mc-dropout-on-pytorch
                """
                ############################################################
                n_batches = len(eval_dataloader)
                forward_passes = 10
                dropout_predictions = np.empty((0, len(to_predict), self.num_labels))
                for i in range(forward_passes):
                    print(f'Forward pass: {i}')
                    ...
                    for i, batch in enumerate(tqdm(eval_dataloader, disable=args.silent)):
                        ...
                        model.eval()
                        # print(f'before enable: {repr(model)}')
                        self.enable_dropout(model)
                        # print(f'after enable: {repr(model)}')
                        # batch = tuple(t.to(device) for t in batch)
                    ...
                    dropout_predictions = np.vstack((dropout_predictions, preds[np.newaxis, :, :]))
                    # print(f'Dropout shape: {dropout_predictions.shape}')
                    
                
                # print(f'Dropout: {dropout_predictions}')
                # print(f'Dropout mean: {np.mean(dropout_predictions, axis=0)}')
                # assert np.mean(dropout_predictions, axis=0) == preds
                preds = np.mean(dropout_predictions, axis=0)
